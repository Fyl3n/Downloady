# Dropload: implementation plan

Dropload is a Droppy droplet that downloads video and audio with
...
| Topic | Decision |
| --- | --- |
| yt-dlp | Dropload downloads its own copy on first run, into `host.environment.containerDirectory/tools/`, using the `yt-dlp_macos.zip` onedir build (starts faster than the onefile build). It is checked against `SHA2-256SUMS`, and Dropload updates it itself. It is **never** placed inside the `.droplet` bundle: Droppy refuses a bundle whose files changed after the user approved it. |
...
All code lives in **one target**, `Sources/Dropload/`. `droppykit build`
links only the `Dropload` module's own objects, so a second library target
would silently be left out of the bundle. Pure logic is tested in
`Tests/DroploadTests` with `swift test`.

```
DroploadDroplet.swift          principal, lifecycle, surface conformances
...
UI/DroploadWidget.swift        shelf widget
UI/DroploadDetailView.swift    expanded surface
UI/DroploadSettingsView.swift  settings pane
```
